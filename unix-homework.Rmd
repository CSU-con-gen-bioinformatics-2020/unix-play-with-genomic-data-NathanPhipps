...
```{bash}
cp fastq/*plate1_[^AB]*R1* fqnotAB
ls {fq_AB,fqnotAB}
```
...
```{bash}
mkdir {dir_A,dir_C,dir_E,dir_G,dir_I}
```
...
```
To make sure you're getting rid of the correct things before they're gone forever, ne'er to return.
```
...
```
One ends in 1, the other ends in 2
```
...
```{bash}
sha1sum R1_all_via_cat.fq copy_of_R1_all_via_cat.fq.gz
```
